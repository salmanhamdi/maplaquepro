# MaPlaquePro — MASTER PLAN v1.5 — VALIDÉ

**Date** : 13 septembre 2026
**Statut** : **VALIDÉ** par le Supervisor (14/09/2026). Référence normative active. Contrôles documentaires : CR-019 à CR-028. L'implémentation n'est engagée que selon les autorisations distinctes du Supervisor (§G.1).
...
- **Master Plan v1.5 (ce document)** : document normatif **validé** par le Supervisor (14/09/2026) ; référence normative active.
...
| **Procédé de fabrication** | Manière physique de fabriquer. **Trois procédés** : (1) gravure laser ; (2) découpe + impression UV ; (3) gravure envers + UV noir. TroLase et TroLase Metallic partagent le procédé « gravure laser ». | P3 D1, D3 |
...
- Plexiglass = **découpe + impression UV**, couleur selon la référence et jamais promise sans référence couleur validée (P7, P9) ;
...
- **Familles MVP** (FACT superviseur ; références exactes À VALIDER) :
  ...
  - **Plexiglass** — procédé : découpe + impression UV — OD-28 / VR-38 ;
...
|---|---|---|---|
| `TROLASE_ENGRAVE` | Gravure laser | 1 gravure laser @ `SPEEDY_400` face · 2 découpe laser @ `SPEEDY_400` face — **opération conditionnelle** (VR-34 : mise au format à l'atelier) | monochrome, gravure |
| `TROLASE_METALLIC_ENGRAVE` | Gravure laser | 1 gravure laser @ `SPEEDY_400` face · 2 découpe laser conditionnelle (VR-34) | monochrome, gravure |
| `PLEXIGLASS_UV` | Découpe + impression UV | 1 découpe laser @ `SPEEDY_400` face · 2 impression UV @ `ARTISJET_3000U` face, **encre dérivée de la politique d'impression de la référence** (noir uniquement ou couleur ; jamais présumée couleur) | impression, traitement couleur selon la politique de la référence (OD-32 / VR-40) |
| `TROGLASS_METALLIC_HYBRID` | Gravure envers + UV noir | 1 gravure laser @ `SPEEDY_400` envers · 2 découpe laser @ `SPEEDY_400` envers, si la géométrie l'exige · 3 impression UV @ `ARTISJET_3000U` envers, encre **noir uniquement** | gravure envers + UV noir envers ; **aucun choix d'encre** |

L'ordre est explicite et figé dans le BAT (`workflowVersion`). **Le moteur de compatibilité raisonne sur le workflow complet.** Tant que la découpe conditionnelle des workflows TroLase n'est pas confirmée (VR-34), le mode sur mesure et les formats nécessitant une découpe sont inactifs pour ces familles (CR-2, ouverte).
...
- **Règle fondamentale (normative, v1.4)** : la fabricabilité est déterminée par l'intersection des contraintes de toutes les opérations du workflow. Exemple normatif : Plexiglass 500 × 300, découpe + impression UV → Speedy OK, ArtisJet NON → `NOT_FABRICABLE`.
...
| 2 | **Matière : famille → référence** | **Quatre cartes de famille** [P3], chacune avec un schéma de coupe et **son procédé** : **TroLase** — « Plaque bicouche », procédé *gravure laser* ; **TroLase Metallic** — « Plaque aspect métallique », procédé *gravure laser* (même procédé que TroLase, famille distincte) ; **Plexiglass** — procédé *découpe + impression UV* ; **TroGlass Metallic Gold / Silver** — procédé *gravure envers + UV noir*. Puis swatches des **références** (apparence complète : surface, finition, couleur révélée le cas échéant). Le client ne doit jamais croire que toutes les familles utilisent le même procédé, ni que deux familles au même procédé sont deux procédés différents. **Aucune étape « finition »** [P4]. | `materialVariantId` |
...
- découpe + impression UV : « Ce visuel sera imprimé par UV — en noir ou en couleur selon la référence choisie » (message adapté à la politique réelle de la référence) ;
...
- découpe + impression UV → artwork en noir ou en couleur selon la politique d'impression, sur le support ;
...
- `TROLASE_ENGRAVE` / `TROLASE_METALLIC_ENGRAVE` → 1 artefact laser (ENGRAVE + HOLES ; CUT si la découpe est confirmée VR-34), côté face.
- `PLEXIGLASS_UV` → 1 artefact laser (CUT + HOLES) + 1 artefact UV (encre dérivée de la politique de la référence, côté face).
...
- Le contenu présente **quatre familles** et **trois procédés** [P3] ; il ne présente jamais toutes les familles comme un seul procédé, ni deux familles au même procédé comme deux procédés différents :
  ...
  - « **Plexiglass** — découpe + impression UV » ;
...
| **E2E** | Parcours par famille : TroLase gravé (sur mesure si VR-34) avec trous ; TroLase Metallic idem ; Plexiglass découpe + impression UV 347 × 490 avec artwork **pour chaque politique d'impression effectivement active** (parcours couleur **seulement si** une référence couleur est active, P9 D2) ; TroGlass Gold hybride avec artwork noir ; refus visible Plexiglass 500 × 300 avec alternative ; refresh / retour ; Stripe test ; noindex ; clavier ; **le client ne voit jamais de choix d'encre** ; **l'orientation de pose n'est jamais affichée** ; modification du payload client ⇒ rejet serveur | Oui |
...
| OD-35 | Découpe laser des workflows TroLase (mise au format) | VALIDATION_REQUIRED (VR-34) | BLOCKING pour le sur mesure TroLase / TroLase Metallic | CR-2 |
...
- Plexiglass = découpe + impression UV, couleur conditionnelle ; tests et GATE 12 couleur conditionnels (P9) ;
...
| CR-2 | Découpe TroLase nécessaire au sur mesure, non confirmée | Non résolue | **Ouverte** (VR-34) |
...
| VR-34 | Mise au format TroLase / TroLase Metallic (découpe) | Sur mesure | Mode sur mesure | Oui (sur mesure) | À VALIDER (↔ OD-35) |
...
| CR-2 | Découpe TroLase / sur mesure | Ouverte (VR-34) | Sur mesure TroLase | — |
...
type ProductionOperation = {
  ...
  condition?: 'always' | 'if_geometry_requires' | 'A_VALIDER'   // découpe TroLase : VR-34
}
...
| E2E | Par famille | TroLase : gravé, format standard (sur mesure si VR-34) avec 4 trous ; TroLase Metallic : idem ; Plexiglass : découpe + impression UV 347 × 490 avec artwork, **un parcours par politique d'impression effectivement active** (couleur **seulement si** une référence couleur est active — P9 D2) ; TroGlass : hybride Gold avec artwork noir, **aucun sélecteur d'encre** ; refus 500 × 300 Plexiglass avec alternative ; **orientation de pose jamais affichée** ; **aucune étape finition** ; libellés « gravure laser » / « découpe + impression UV » / « gravure envers + UV noir » ; panier « 2 trous à X mm » ; mobile + desktop ; refresh / retour ; clavier |
...
**MASTER PLAN v1.5 — VALIDÉ (14/09/2026). IMPLÉMENTATION UNIQUEMENT SELON LES AUTORISATIONS DU SUPERVISOR.**
*Fin du Master Plan v1.5 — MaPlaquePro.*
